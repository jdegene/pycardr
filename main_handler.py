...
def get_darabanth(work_fol=work_fol, conDB=conDB, c=c, searchterm_json=searchterm_json):
    ...
    # use only this "search_phrases" (i.e. category) for now
    for search_phrase in ['Topics_156997', 'Themen~2000001',  
                          'Deutschland~2000040','Postkartensammlungen-Lose~2000004']:
        ...
        while existing_share < 0.9:
            cur_page = cur_page + 1
            get_page = website_handling.darabanth(page=cur_page, search_phrase=search_phrase)
...
def get_invaluable(work_fol=work_fol, conDB=conDB, c=c, searchterm_json=searchterm_json):
    ...
    while existing_share < 0.9:
        cur_page = cur_page + 1
        get_page = website_handling.invaluable(page=cur_page, mode='main')
        ...
        for image in get_page:
            imageID_found = image_handling.checkID(image['entry_id'], c, subsite='invaluable')
            exisiting_imgs_int = exisiting_imgs_int + min(imageID_found, 1)
            existing_share = exisiting_imgs_int / len(get_page)             
        ...
        get_subsites = website_handling.invaluable(page=cur_page, mode='single', pass_list = pass_list)
        ...
        for subsite_image in get_subsites:
            ...
            lowest_dhash = image_handling.check_all_hashes(img, 
                             (subsite_image['thumb_url'], subsite_image['entry_url'],
                              'invaluable', "", subsite_image['entry_id']), 
                             work_fol,
                             c, 
                             conDB, 
                             threshold=13)      
...
def get_philasearch(work_fol=work_fol, conDB=conDB, c=c):

    search_list = searchterm_json['philasearch']

    for search_term in search_list:
        cur_page = 0 # starting page to crawl (will instantly incremented by 1, so 0 is first page)
        existing_share = 0    

        while existing_share < 0.95:
            cur_page = cur_page + 1
            get_page = website_handling.philasearch(search_term, page=cur_page)
            
            if len(get_page) < 4:
                print("Leaving page, <4 items found")
                break
            ...
            for image in get_page:    
                ...
                # return 0 if image id is not in database yet
                imageID_found = image_handling.checkID(image['entry_id'], c, subsite='philasearch')
                exisiting_imgs_int = exisiting_imgs_int + min(imageID_found, 1)
                existing_share = exisiting_imgs_int / len(get_page)  
                ...
                if imageID_found == 0:
                    ...
                    lowest_dhash = image_handling.check_all_hashes(img, 
                                     (image['thumb_url'], image['entry_url'],
                                      'philasearch', search_term, image['entry_id']), 
                                     work_fol,
                                     c, 
                                     conDB, 
                                     threshold=13)                
